...
//	}
	...
	@PostMapping(value = "/guardar/{rol}", consumes = "application/json")
	public ResponseEntity<?> guardar(
			@ApiParam(value = "Objeto de tipo estudiante", required = true) @Valid @RequestBody Usuario usuario , @PathVariable int rol) throws ConflictException {
		service.guardar(usuario , rol);
		return new ResponseEntity<Object>(HttpStatus.CREATED);
	}
	
	@GetMapping(value = "/obtenerPaginado" ,produces = "application/json")
	public ResponseEntity<?> retonarPaginado(Pageable page) {
		Page<Usuario> listaAutor = service.retornarPaginado(page);
		return new ResponseEntity<Page<Usuario>>(listaAutor, HttpStatus.OK);	
	}
